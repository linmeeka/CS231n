...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        #forward
        h0,cache_features=affine_forward(features,W_proj,b_proj)
        
        captions_in_emb,cache_in_emb=word_embedding_forward(captions_in,W_embed)
        
        if self.cell_type=='rnn':
            h,cache_rnn=rnn_forward(captions_in_emb,h0,Wx,Wh,b)
        if self.cell_type=='lstm':
            h,cache_rnn=lstm_forward(captions_in_emb,h0,Wx,Wh,b)
        ...
        if self.cell_type=='rnn':
            drnn,dh0,grads['Wx'],grads['Wh'],grads['b']=rnn_backward(dscore,cache_rnn)
        if self.cell_type=='lstm':
            drnn,dh0,grads['Wx'],grads['Wh'],grads['b']=lstm_backward(dscore,cache_rnn)
            
        grads['W_embed']=word_embedding_backward(drnn,cache_in_emb)
        
        dfeatures,grads['W_proj'],grads['b_proj']=affine_backward(dh0,cache_features)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
